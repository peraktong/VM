...
class Fit_Bolshoi_tree():
    ...
    def read_tree(self):
        ...
        hmf_values = pickle.load(pkl_file)
        pkl_file.close()
        ...
        for i in range(0, 31):
            ...
            mass_tree = min+0.05+i*0.1


            mass_tree = "{0:.2f}".format(mass_tree)


            data_path = "Analytic_trees_100"+"M_"+str(mass_tree)+".pkl"
            ...
            # attention!
            merger_tree_all.extend(array_i.T)

            hmf_weight_array.extend(np.ones(len(array_i[0,:]))*hmf_values[i])

            #print(len(hmf_weight_array))


        merger_tree_all = np.array(merger_tree_all).T

        #print(merger_tree_all.shape)

        hmf_weight_array = np.array(hmf_weight_array)
        halo_number_array = np.ones(len(hmf_weight_array))



        halo_number_array = halo_number_array*hmf_weight_array

        self.halo_number_array = halo_number_array

        #print(hmf_weight_array.shape)
        ...
        pkl_file = open("a_target_analytic.pkl", 'rb')
        ...
        ones_2d = np.ones([len(self.a_target), len(self.a_target)])
    ...
    def fit_quenched_fraction(self):
        ...
        delta_ms_3d = np.repeat(delta_ms_all[:, :, np.newaxis], len(self.a_target), axis=2)
        ...
        # weighted by halo number array!

        judge_array = judge_array*self.halo_number_array
        ...
        for m in range(0, 19):
            ...
            fraction_array.append(np.nansum(judge_array[mask]) /np.nansum(self.halo_number_array[mask]))
    ...
    def fit_behroozi(self):
        ...
        # fit from 11.0 to 12.5 with bin size=0.5??
        ...
        for i in range(0,5):
            ...
            slope = np.sum(log_Behroozi_Ms[a_index_Behroozi] * log_median_Ms[a_index_us]) / np.sum(log_median_Ms[a_index_us] ** 2)
            # slope=1
...
kwargs = {"MCMC": None, "A1": None, "A2": None, "A3": None, "f0_log": None, "A4": None, "A5": None, "A6": None, "mht": None,
          "mst": None, "zc": None, "sigmaz": None,
          "alphaz": None, "mhc_log": None, "msc_log": None, "sigmah": None, "sigmag": None, "alphah": None, "alphag": None}
...
# Here we fit f0, mst,sigmag and alphag.
...
def lnlike(theta, x, y):
    ...
    print(f0_log,mhc_log,sigmah,alphah)


    chi_q = model.fit_quenched_fraction()
    chi_b = model.fit_behroozi()
...
def lnprior(theta):
    ...
    #
    # -3 < f0_log < 0 and
    if -3 < f0_log < 0 and 8 < mhc_log < 13 and 0.01 < sigmah < 3 and -5 < alphah < 5:
...
pos = [
    [kwargs["f0_log"],kwargs["mhc_log"], kwargs["sigmah"], kwargs["alphah"]] + 1e-3 * np.random.randn(ndim) for i in range(nwalkers)]
...
save_path = "Analytic_trees_Matrix_best_fit_v1.pkl"
...
f0_log,mhc_log,sigmah,alphah = result[i, j, :]
...
result = op.minimize(nll,
                     [kwargs["f0_log"],kwargs["mhc_log"], kwargs["sigmah"], kwargs["alphah"]],
                     args=(x, y))
