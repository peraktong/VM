...
def lnprior(theta):
    ...
    #
    # -3 < f0_log < 0 and
    if -3 < f0_log < 0 and 8 < mhc_log < 13 and 0 < sigmah < 3 and -5 < alphah < 5:
        return 0.0
    return -np.inf
...
def lnprob(theta, x, y):
    ...
    elif math.isnan(lp + lnlike(theta, x, y))==True:
        return -np.inf, str(lnlike(theta, x, y) / (-0.5))
    """
...
for ii in range(0, 14):
    ...
    pos = [
        [kwargs["f0_log"], kwargs["mhc_log"], kwargs["sigmah"], kwargs["alphah"]] + 1e-3 * np.random.randn(ndim) for i
        in range(nwalkers)]
    ...
    save_path = "Analytic_trees_sep_best_fit_v1.pkl".replace("best_fit_", "best_fit_" + str(11.0 + ii * 0.2) + "_sep_")
